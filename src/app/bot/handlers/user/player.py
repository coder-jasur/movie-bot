...
from src.app.database.queries.movie.multi_films import MultiFilmSeriesActions, MultiFilmMiniSeriesActions
from src.app.database.queries.movie.anime import AnimeSeriesActions, AnimeMiniSeriesActions
from src.app.bot.keyboards.callback_data import SeriesPlayerCD, FeatureFilmPlayerCD, MiniSeriesPlayerCD, ActionType
from src.app.bot.keyboards.inline import series_player_kbd, film_kbd, mini_series_player_kbd
...
async def series_player(call: CallbackQuery, session: AsyncSession, callback_data: SeriesPlayerCD):
    ...
    is_vip = db_user.vip_status == "active" if db_user else False
    ...
    if series_data: actions = SeriesActions(session)
    if not actions:
        series_data = await MultiFilmSeriesActions(session).get_series(code)
        if series_data: actions = MultiFilmSeriesActions(session)
    if not actions:
        series_data = await AnimeSeriesActions(session).get_series(code)
        if series_data: actions = AnimeSeriesActions(session)
    ...
    from src.app.bot.common.utils import get_user_language, resolve_movie_media, get_thumbnail_input
    ...
    filtered_series_data = [
        ...
        if (isinstance(s.files, dict) and target_language_req in s.files) or 
           (isinstance(s.language, str) and target_language_req in (s.language or "").split(","))
    ]
    ...
    current_series = next(
        (s for s in filtered_series_data
    ...
    current_index = next(
        (i for i, s in enumerate(filtered_series_data, start=1)
         if s.season == callback_data.season_number and s.series == callback_data.series_number),
        1
    )

    series_count_for_current_season = sum(1 for s in filtered_series_data if s.season == callback_data.season_number)
    series_count = len(filtered_series_data)
    seasons_count = len(set(s.season for s in filtered_series_data)) if filtered_series_data else 0
    ...
    if callback_data.action == ActionType.set_quality and not is_vip and callback_data.quality in ["720p", "1080p"]:
    ...
    file_id, name, caption, target_language_res, target_quality, files, _captions, thumbnail_id = resolve_movie_media(current_series, target_language_req, callback_data.quality, is_vip=is_vip)
            ...
            await call.message.edit_reply_markup(
                reply_markup=_build_series_list_menu(
                    ...
                    cd_builder=lambda s: SeriesPlayerCD(
                        code=code, series_number=s, season_number=callback_data.season_number,
                        all_series_numebr=current_series.series,
                        action=ActionType.next_series if s > callback_data.series_number else ActionType.back_series,
                        quality=target_quality, language=target_language_res).pack(),
                    back_action=True
                )
            )
        return
        ...
        with suppress(TelegramBadRequest):
            await call.message.edit_reply_markup(
                reply_markup=_build_seasons_list_menu(
                    ...
                    cd_builder=lambda s: SeriesPlayerCD(
                        code=code, series_number=1, season_number=s,
                        all_series_numebr=current_series.series,
                        action=ActionType.next_season if s > callback_data.season_number else ActionType.back_season,
                        quality=target_quality, language=target_language_res).pack(),
                    back_action=True
                )
            )
    ...
    if callback_data.action in [ActionType.open_quality_menu, ActionType.close_quality_menu, ActionType.open_language_menu, "save_to_favorites", "remove_in_favorites"]:
        with suppress(TelegramBadRequest):
            await call.message.edit_reply_markup(
...
async def feature_movies_player(call: CallbackQuery, callback_data: FeatureFilmPlayerCD, session: AsyncSession):
    ...
    if not movie:
        movie = await MultiFilmFeatureActions(session).get_feature_film(callback_data.code)
    if not movie:
    ...
    target_language_req = callback_data.language or user_lang
    if callback_data.actions == ActionType.set_quality and not is_vip and callback_data.quality in ["720p", "1080p"]:
        from src.app.bot.handlers.user.account import vip_tarif_handler
        await call.answer(str(_("💎 VIP obuna talab qilinadi")), show_alert=False)
        await vip_tarif_handler(call.message)
        return

    file_id, name, caption, target_language_res, target_quality, files, _captions, thumbnail_id = resolve_movie_media(movie, target_language_req, callback_data.quality, is_vip=is_vip)
    ...
    if callback_data.actions in [ActionType.open_quality_menu, ActionType.close_quality_menu, ActionType.open_language_menu, "add_to_favorites", "delete_for_favorites"]:
        with suppress(TelegramBadRequest):
            await call.message.edit_reply_markup(
...
async def mini_series_player(call: CallbackQuery, callback_data: MiniSeriesPlayerCD, session: AsyncSession):
    ...
    if mini_series_data: actions = MiniSeriesActions(session)
    if not actions:
        mini_series_data = await MultiFilmMiniSeriesActions(session).get_mini_series(code)
        if mini_series_data: actions = MultiFilmMiniSeriesActions(session)
    if not actions:
        mini_series_data = await AnimeMiniSeriesActions(session).get_mini_series(code)
        if mini_series_data: actions = AnimeMiniSeriesActions(session)
    ...
    current_series = next(
        (s for s in filtered_mini_series_data if s.series == callback_data.series_number),
        None
    )
            ...
            await call.message.edit_reply_markup(
                reply_markup=_build_series_list_menu(
                    ...
                    cd_builder=lambda s: MiniSeriesPlayerCD(
                        code=code, series_number=s,
                        action=ActionType.next_series if s > current_series.series else ActionType.back_series,
                        quality=target_quality, language=target_language_res).pack(),
                    back_action=True
                )
            )
    ...
    if callback_data.action in [ActionType.open_quality_menu, ActionType.close_quality_menu, ActionType.open_language_menu, "add_to_favorites", "delete_for_favorites"]:
        with suppress(TelegramBadRequest):
            await call.message.edit_reply_markup(
