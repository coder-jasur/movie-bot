...
referral_dialog = Dialog(
    Window(
        ...
        SwitchTo(Const(str(_("➕ Создать реферала"))), id="add_ref", state=ReferralSG.add),
        Cancel(Const(str(_("⬅️ Назад"))), id="back"),
        state=ReferralSG.menu,
        getter=get_referrals,
    ),
    Window(
        Const(str(_("✏️ <b>Новый реферал:</b>\n\nВведите название:"))),
    ...
    Window(
        Format(_("ℹ️ <b>Информация о реферале:</b>\n\n"
        ...
        Button(Const(str(_("🗑 Удалить"))), id="delete_ref", on_click=on_referral_delete),
