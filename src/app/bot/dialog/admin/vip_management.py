...
from aiogram_dialog.widgets.kbd import Button, Row, SwitchTo, Cancel, Back
...
async def get_user_data(dialog_manager: DialogManager, **kwargs):
    ...
    is_vip = user.vip_status == "active" and (user.vip_expires_at is None or user.vip_expires_at > now)
    
    expiry_str = _("Cheksiz") if user.vip_expires_at is None else user.vip_expires_at.strftime('%d.%m.%Y %H:%M')
...
vip_management_dialog = Dialog(
    ...
    Window(
        Format(_("👤 <b>Foydalanuvchi ma'lumotlari:</b>\n\n🆔 ID: <code>{tg_id}</code>\n👤 Username: @{username}\n💎 VIP: {vip_status_text}\n📅 Muddati: {expiry}")),
        Row(
        ...
        Row(
            ...
            Button(Format("{grant_unlimited_btn}"), id="grant_unlimited", on_click=grant_vip),
        ),
