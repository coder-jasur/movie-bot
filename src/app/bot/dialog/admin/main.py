...
from src.app.bot.states.admin.dialogs import AdminMenuSG, AddMovieWizardSG, EditMovieSG, BackupSG, AdminVIPManagerSG, AdminManagementSG
...
async def get_statistics(dialog_manager: DialogManager, **kwargs):
    ...
    langs_str = "\n".join([
        f"   • {get_flag_emoji(l['code'])}: {l['count']}"
    ...
    return {
        ...
        "rev_day": fmt_rev("day"),
        "rev_week": fmt_rev("week"),
        "rev_month": fmt_rev("month"),
        "rev_year": fmt_rev("year"),
        "rev_total": fmt_rev("total"),
...
async def get_labels(dialog_manager: DialogManager, **kwargs):
    ...
    status_text = _("🟢 Worker: Onlayn") if worker_online else _("🔴 Worker: Offlayn (Lokal kompyuter yoqilmagan)")
    
    return {
        "title": f"<b>{_('ADMIN_PANEL_TITLE')}</b>\n\n{status_text}",
        "add": _("BTN_ADD_MOVIE"),
        "edit": _("BTN_EDIT_DELETE"),
        "channels": _("BTN_CHANNELS_BOTS"),
        "broadcast": _("BTN_SEND_BROADCAST"),
        "referrals": _("BTN_REFERRALS"),
        "stats": _("BTN_STATISTICS"),
        "vip_manage": _("BTN_VIP_MANAGEMENT"),
        "admin_manage": _("👥 Adminlarni boshqarish"),
        "backup": _("BTN_BACKUP"),
        "close": _("BTN_CLOSE"),
        "cancel": _("BTN_CANCEL"),
        "confirm": _("BTN_CONFIRM_SEND"),
        "broadcast_input_text": _("📨 <b>Broadcast</b>\n\nBarcha foydalanuvchilarga yuboriladigan xabarni yuboring:\n<i>(matn, rasm, video — istalgan format)</i>"),
        "broadcast_confirm_text": _("⚠️ <b>Broadcast tasdiqlash</b>\n\nXabarni barcha foydalanuvchilarga yuborishni tasdiqlaysizmi?"),
...
async def on_add_movie_click(c: CallbackQuery, widget: Button, manager: DialogManager):
    if not is_worker_online():
        await c.answer(str(_("⚠️ Lokal kompyuterda Worker ishga tushirilmagan! Iltimos, oldin workerni yoqing.")), show_alert=True)
        return
...
async def run_broadcast_task(bot: Bot, session_pool, admin_id: int, from_chat_id: int, message_id: int):
    try:
...
async def on_broadcast_confirm(c: CallbackQuery, widget, manager: DialogManager):
    ...
    if not message_id or not chat_id:
        await c.answer(str(_("❌ Xabar topilmadi")), show_alert=True)
        return
    ...
    await c.answer(str(_("🚀 Xabar yuborish fonda boshlandi. Tez orada hisobot olasiz.")), show_alert=True)
...
admin_main_dialog = Dialog(
    Window(
        ...
        Row(
            ...
            SwitchTo(Format("{broadcast}"), id="broadcast", state=AdminMenuSG.broadcast_input),
        ),
        Row(
        ...
        Row(
            ...
            Start(Format("{vip_manage}"), id="vip_manage", state=AdminVIPManagerSG.search),
        ),
        Row(
            Start(Format("{admin_manage}"), id="admin_manage", state=AdminManagementSG.list_admins, when="is_super_admin"),
        ),
        Row(
    ...
    Window(
        ...
        Button(Format("{confirm}"), id="confirm_broadcast", on_click=on_broadcast_confirm),
