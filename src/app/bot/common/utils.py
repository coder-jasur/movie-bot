...
async def is_active_vip(user, session: AsyncSession = None) -> bool:
    ...
    if session:
        ...
        if await AdminActions(session).is_admin(user.tg_id):
            return True
...
def build_movie_caption(movie, lang: str) -> str:
    ...
    if "mini" in movie.__class__.__name__.lower():
        icon = "🧩"
...
def resolve_movie_media(
    ...
    if isinstance(files, dict):
        ...
        for lang_key, content in files.items():
            ...
            if short_lang not in normalized_files:
                normalized_files[short_lang] = {}
            if isinstance(content, dict):
                normalized_files[short_lang].update(content)
    ...
    if not target_lang:
        ...
        return (
            None,
            get_localized_name(movie, user_lang),
            build_movie_caption(movie, user_lang),
            user_lang,
            None,
            {},
            {},
    ...
    available_ranks = sorted(
        [(q, get_quality_rank(q)) for q in lang_files.keys()],
        key=lambda x: x[1],
        reverse=True,
    )
    ...
    if not final_caption:
        final_caption = flat_captions.get(target_lang)
    if not final_caption and flat_captions:
        final_caption = next(
            (v for v in flat_captions.values() if isinstance(v, str) and v.strip()),
            None,
        )
    if not final_caption:
    ...
    # 6. Resolve Thumbnail
    flat_thumbnails = (
        {get_lang_code(k): v for k, v in thumbnails.items() if isinstance(v, str)}
        if isinstance(thumbnails, dict)
        else {}
    )
    thumbnail_id = flat_thumbnails.get(user_lang) or flat_thumbnails.get(target_lang)
    if not thumbnail_id and flat_thumbnails:
        thumbnail_id = next(iter(flat_thumbnails.values()))
...
def get_localized_name(movie, user_lang: str) -> str:
    ...
    data = getattr(
        movie, "name", movie.get("name") if isinstance(movie, dict) else movie
    )
    ...
    if isinstance(data, dict):
        if selected_lang:
            ...
            if short_code in data and data[short_code]:
                flag = next(
                    (l["flag"] for l in LANGUAGES if l["id"] == short_code), "🌐"
                )
        ...
        for lang, val in sorted(data.items()):
            if not val:
                continue
            flag = next(
                (l["flag"] for l in LANGUAGES if l["id"] == get_lang_code(lang)), "🌐"
            )
