...
async def get_thumbnail_input(bot, file_id: str):
    ...
    try:
        ...
        if os.path.exists(watermark_path):
            ...
            try:
                ...
                if proc.returncode == 0:
                    logger.info(
                        f"get_thumbnail_input: applied watermark to {output_path}"
                    )
                    return FSInputFile(output_path, filename="thumbnail.jpg")
                else:
                    logger.warning(
                        f"get_thumbnail_input: ffmpeg watermark failed with code {proc.returncode}, stderr: {stderr.decode().strip()}, using raw image"
                    )
            except Exception as ffmpeg_e:
                logger.warning(
                    f"get_thumbnail_input: ffmpeg execution failed: {ffmpeg_e}, using raw image"
                )
        ...
        logger.warning(
            f"get_thumbnail_input failed for {file_id[:20] if file_id else 'None'}: {e}"
        )
        return None
...
async def is_active_vip(user, session: AsyncSession = None) -> bool:
    """
    Checks if a user has an active VIP status based on both
    the vip_status flag and the vip_expires_at timestamp.
    Admins have permanent VIP status.
    """
    if not user:
        return False

    # 1. Check if user is a super-admin in config
    from src.app.core.config import load_config
    config = load_config()
    if user.tg_id in config.admins_ids:
        return True

    # 2. Check if user is an admin in DB
    if session:
        from src.app.database.queries.admin import AdminActions
        admin_actions = AdminActions(session)
        if await admin_actions.is_admin(user.tg_id):
            return True
...
def get_lang_code(label: str) -> str:
    ...
    if not label:
        return "uz"
    l = str(label).lower()
    if "o'zbek" in l or "uz" in l:
        return "uz"
    if "rus" in l or "ru" in l:
        return "ru"
    if "ingliz" in l or "en" in l or "english" in l:
        return "en"
    return l
...
def resolve_movie_media(
    ...
    is_flat = all(
        str(k).lower().endswith("p") or str(k).lower() in qualities_keywords
    ...
    for lang_key, content in raw_files.items():
        ...
        if short_code in normalized_files:
            ...
            if isinstance(existing, dict) and isinstance(content, dict):
                ...
                normalized_files[short_code] = (
                    content if isinstance(content, dict) else existing
                )
        else:
    ...
    if not target_lang:
        ...
        return (
            getattr(movie, "video_file_id", None),
            getattr(movie, "name", "No Name"),
            getattr(movie, "captions", "No caption"),
            "uz",
            "original",
            {},
            {},
    ...
    if lang_files:
        ...
        file_id = lang_files.get(
            target_quality,
            lang_files.get("original", getattr(movie, "video_file_id", None)),
        )
    else:
    ...
    final_thumbnails = {
        get_lang_code(k): v for k, v in raw_thumbnails.items() if isinstance(v, str)
    }
...
async def send_admin_preview_media_group(
    bot, chat_id: int, file_id: str, thumbnail_id: str, caption: str = None
):
    ...
    if thumbnail_id:
        media.append(
            InputMediaPhoto(media=thumbnail_id, caption=caption, parse_mode="HTML")
        )
